...
async function getCompletedData(page: number = 1) {
  try {
    const [listData, topAnimeData, genresResponse, topAiringData, mostPopularData, mostFavoriteData, completedCategoryData] = await Promise.all([
      api.completed(page),
      api.topAnime(),
      api.genres(),
      api.topAiring(1),
...
export default async function CompletedPage({
  ...
  const { listData, topAnimeData, genresData, topAnimeCategoriesData } = await getCompletedData(page);
