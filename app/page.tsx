...
async function fetchHomeData() {
  const [
    ...
    api
      .topAnime()
      .catch(() => ({ top_today: [], top_week: [], top_month: [] })),
    api
      .genres()
      .then((r) => r.genres)
      .catch(() => []),
  ...
  if (Array.isArray(trendingRaw)) {
    ...
    Array.isArray((trendingRaw as { trending?: TrendingItem[] }).trending)
  ) {
    ...
    <div className="relative">
      <main className="pb-20">
        ...
        <section className="pt-20">
          <Suspense
            fallback={
              <div className="h-[480px] md:h-[540px] glass animate-pulse rounded-none" />
            }
          >
            {spotlight.length ? (
              <HeroCarousel items={spotlight} />
            ) : (
              <div className="h-[480px] md:h-[540px] glass flex items-center justify-center text-sm text-white/50">
                No spotlight data.
              </div>
            )}
          </Suspense>
