...
export function UserProfilePage({ username }: UserProfilePageProps) {
  ...
  const [listCounts, setListCounts] = useState<Record<AnimeListStatus | 'total', number> | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<AnimeListStatus | 'all'>('all');

  const isOwnProfile = user && userProfile?.username?.toLowerCase() === username.toLowerCase();
      ...
      <div className="min-h-screen bg-black pt-20">
        ...
        <div className="container-padded py-12 text-center">
          <div className="glass rounded-2xl p-12 max-w-md mx-auto">
            ...
            <p className="text-white/50 text-sm mb-6">The user &quot;{username}&quot; doesn&apos;t exist or has a private profile.</p>
            <Link href="/" className="inline-flex items-center gap-2 px-5 py-2.5 glass rounded-xl text-white hover:bg-white/10 transition-all text-sm">
              {Icons.arrowLeft}
              Back to Home
            </Link>
          </div>
  ...
  const getFilteredAnime = () => {
    ...
    if (activeTab === 'all') {
      return Object.values(animeLists).flat().sort((a, b) => b.updatedAt - a.updatedAt);
    }
        ...
        <div className="glass rounded-2xl p-6 md:p-8 mb-6">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-white/40 text-sm mb-3">@{profileUser.username}</p>
              {profileUser.bio && (
                <p className="text-white/60 text-sm mb-4 max-w-2xl leading-relaxed">{profileUser.bio}</p>
              )}
        ...
        <div className="mb-6 -mx-4 px-4 md:mx-0 md:px-0">
          <div className="flex gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0 scrollbar-hide snap-x snap-mandatory md:snap-none touch-pan-x md:flex-wrap">
            ...
            >
              ...
              <span className="ml-1.5 text-xs md:text-sm opacity-70">{listCounts?.total || 0}</span>
            </button>
            {statuses.map((status) => (
              <button
                key={status}
              ...
              >
                <span className="text-xs md:text-sm font-medium">{LIST_STATUS_LABELS[status]}</span>
                <span className="ml-1.5 text-xs md:text-sm opacity-70">{listCounts?.[status] || 0}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          {getFilteredAnime().length === 0 ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 sm:gap-3">
              {getFilteredAnime().map((anime) => (
                <Link key={anime.animeId} href={anime.animeId} className="group">
                  <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-white/5">
                    ...
                    <div className={`absolute top-1.5 right-1.5 p-1.5 rounded-lg bg-black/60 backdrop-blur-sm ${
                      anime.status === 'watching' ? 'text-blue-400' :
                      anime.status === 'completed' ? 'text-green-400' :
                      anime.status === 'on-hold' ? 'text-yellow-400' :
                      anime.status === 'dropped' ? 'text-red-400' :
                      'text-purple-400'
                    }`}>
                      {STATUS_ICONS[anime.status as AnimeListStatus]}
                    </div>
                    <div className="absolute bottom-0 left-0 right-0 p-2">
                      <p className="text-white text-[11px] font-medium line-clamp-2">{anime.title}</p>
                    </div>
                  </div>
                </Link>
              ))}
