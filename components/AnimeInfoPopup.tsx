...
export function AnimeInfoPopup({ qtip, poster, slug, isVisible, position, isSidebar = false }: AnimeInfoPopupProps) {
  ...
  const shouldPositionAbove = position.y + popupHeight / 2 > viewportHeight - 50;
        ...
        <div className="p-4 pt-0 space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="px-2 py-1 font-medium flex items-center gap-1">
              <Image src="/star.svg" alt="star" width={20} height={20} className="star" />
              {qtip.rating || "N/A"}
            </span>
            ...
            {qtip.dub && qtip.dub !== "0" && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-medium bg-[linear-gradient(to_right,rgba(16,185,129,0.25),rgba(16,185,129,0.08))] ring-1 ring-emerald-500/40 text-emerald-200 shadow-[0_0_0_1px_rgba(16,185,129,0.25)] backdrop-blur-sm">
                <Image src="/mic.svg" alt="mic" width={12} height={12} className="brightness-0 invert" />
                {qtip.dub}
              </span>
            )}
