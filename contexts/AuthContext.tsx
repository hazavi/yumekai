...
interface AuthContextType {
  ...
  updateProfilePhoto: (file: File) => Promise<{ success: boolean; error?: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const createOrUpdateUserProfile = async (firebaseUser: User, additionalData?: Partial<UserProfile>) => {
    if (!database) return;
    ...
    if (!snapshot.exists()) {
      // Generate unique username
      const baseUsername = firebaseUser.displayName?.toLowerCase().replace(/[^a-z0-9_]/g, '_') || `user_${firebaseUser.uid.slice(0, 8)}`;
      ...
      await set(ref(database, `usernames/${username.toLowerCase()}`), firebaseUser.uid);
  ...
  const fetchUserProfile = async (uid: string, firebaseUser?: User | null) => {
    ...
    if (snapshot.exists()) {
      ...
      if (profile.username) {
        const usernameRef = ref(database, `usernames/${profile.username.toLowerCase()}`);
      ...
      const baseUsername = firebaseUser.displayName?.toLowerCase().replace(/[^a-z0-9_]/g, '_') || `user_${uid.slice(0, 8)}`;
      const username = await generateUniqueUsername(baseUsername);
  ...
  const canChangeUsername = () => {
    ...
    const daysRemaining = Math.ceil((thirtyDaysMs - timeSinceChange) / (24 * 60 * 60 * 1000));
  ...
  const updateUsername = async (newUsername: string): Promise<{ success: boolean; error?: string }> => {
    ...
    if (!available && newUsername.toLowerCase() !== userProfile.username?.toLowerCase()) {
    ...
    try {
      ...
      await set(ref(database, `usernames/${newUsername.toLowerCase()}`), user.uid);
  ...
  const canChangeDisplayName = () => {
    ...
    const daysRemaining = Math.ceil((fiveDaysMs - timeSinceChange) / (24 * 60 * 60 * 1000));
    return { canChange: false, daysRemaining };
  };
  ...
  const updateDisplayName = async (newDisplayName: string): Promise<{ success: boolean; error?: string }> => {
    ...
    if (!canChange) {
      return { success: false, error: `You can change your display name again in ${daysRemaining} days` };
    }
  ...
  const updateProfilePhoto = async (file: File): Promise<{ success: boolean; error?: string }> => {
    if (!user || !database) {
      return { success: false, error: 'Not authenticated' };
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      return { success: false, error: 'Please upload a valid image (JPEG, PNG, GIF, or WebP)' };
    }
